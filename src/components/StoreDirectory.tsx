...
export const StoreDirectory: React.FC<StoreDirectoryProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl bg-[#0e1320] border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Targeting Cedar Falls, Waterloo, Cedar Rapids, Coralville, Iowa City, Williamsburg, Ankeny & surrounding hubs
            </p>
          </div>
        </div>

        <div className="flex items-center gap-3 font-mono text-xs text-slate-300">
          <div className="px-3.5 py-2 rounded-2xl bg-[#07090e] border border-slate-800">
            Matching: <strong className="text-amber-400 font-bold">{filteredStores.length}</strong> / 102
          </div>
          <div className="px-3.5 py-2 rounded-2xl bg-[#07090e] border border-slate-800 hidden sm:block">
      ...
      {/* Filter Controls */}
      <div className="p-5 rounded-3xl bg-[#0e1320] border border-slate-800/90 space-y-4 shadow-lg">
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
          ...
          <div className="sm:col-span-3">
            <select
              ...
              className="w-full px-3.5 py-2.5 rounded-2xl bg-[#07090e] border border-slate-800 text-slate-300 text-xs focus:outline-none focus:border-amber-400 transition-colors"
            >
              {TIERS.map((t) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredStores.map((store) => (
          <div
            ...
            className="flex flex-col justify-between p-5 rounded-3xl bg-[#0e1320] border border-slate-800/90 hover:border-amber-500/40 hover:shadow-xl transition-all"
          >
            <div>
              {/* Top Rank & Distance */}
              <div className="flex items-start justify-between gap-2 mb-2.5">
                <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-bold bg-[#07090e] text-slate-300 border border-slate-800 font-mono">
                  Rank #{store.rank}
                </span>
                <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-bold bg-amber-400/10 text-amber-400 border border-amber-400/20 font-mono flex items-center gap-1">
                  <Car className="w-3 h-3 inline" />
                  {store.distanceMiles} mi away
                </span>
              </div>

              <h4 className="font-extrabold text-base text-white mb-1 font-display">
                {store.name}
              </h4>
              <p className="text-xs text-slate-400 flex items-center gap-1 mb-3.5">
                <MapPin className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                <span className="truncate">{store.address}, {store.city}, {store.state}</span>
              </p>

              {/* Tier & Category Info */}
              <div className="p-3 rounded-2xl bg-[#07090e] border border-slate-800/80 text-xs space-y-1.5 mb-3.5">
                ...
                {store.markdownDay && (
                  <div className="text-slate-300 flex items-center gap-1.5 pt-1 border-t border-slate-800/60">
                    <Calendar className="w-3.5 h-3.5 text-emerald-400" />
                    <span className="text-slate-400">Markdown Day: </span>
                    <strong className="text-emerald-400 font-semibold">{store.markdownDay}</strong>
                  </div>
                )}
              </div>
              ...
              {store.clearanceTip && (
                <div className="p-3 rounded-2xl bg-amber-950/20 border border-amber-500/30 text-xs text-amber-200/90 leading-relaxed mb-3.5">
            ...
            <div className="pt-3 border-t border-slate-800/80 grid grid-cols-2 gap-2">
              <button
                ...
                className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold text-center transition-all"
              >
                View Deals
              </button>
              <button
                onClick={() => onEvaluateStoreSku(store.name)}
                className="px-3 py-2 rounded-xl bg-amber-400/20 hover:bg-amber-400 text-amber-300 hover:text-slate-950 text-xs font-bold text-center border border-amber-400/40 transition-all flex items-center justify-center gap-1.5"
              >

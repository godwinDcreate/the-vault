...
export const TripOptimizerModal: React.FC<TripOptimizerProps> = ({ onOpenListing }) => {
  ...
  const presetTrips = {
    cedar_valley: {
      name: 'Cedar Falls & Waterloo Local Sprint (18 Miles)',
    ...
    cedar_rapids_loop: {
      name: 'Hwy 218 South Corridor to Cedar Rapids (64 Miles)',
    ...
    coralville_loop: {
      name: 'I-380 South to Coralville & Iowa City Hub (88 Miles)',
    ...
    full_circuit: {
      name: 'Iowa Weekend Super-Haul Circuit (142 Miles)',
  ...
  return (
    <div className="space-y-6">
      {/* Route Header Banner */}
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-blue-950/40 via-[#0e1320] to-[#0e1320] border border-blue-500/30 shadow-xl">
        ...
        {/* Route Selectors */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-6 pt-5 border-t border-blue-500/20">
            ...
            <button
              ...
              className={`p-3.5 rounded-2xl text-left transition-all border ${
                selectedCityZone === key
                  ? 'bg-blue-500 text-slate-950 border-blue-400 font-extrabold shadow-lg'
                  : 'bg-[#07090e] text-slate-300 border-slate-800 hover:border-slate-700 hover:text-white'
              }`}
            >
              <div className="text-xs font-bold truncate font-display">{presetTrips[key].name.split('(')[0]}</div>
              <div className={`text-[11px] font-mono mt-1 ${
                selectedCityZone === key ? 'text-slate-950 font-bold' : 'text-slate-400'
              }`}>
      ...
      <div className="p-6 rounded-3xl bg-[#0e1320] border border-slate-800 space-y-4 shadow-xl">
        <h3 className="font-extrabold text-base text-white flex items-center justify-between font-display">
          <span>Turn-by-Turn Route Stops & Clearance Endcap Targets</span>
          <span className="text-xs font-mono text-slate-400 bg-[#07090e] px-2.5 py-1 rounded-xl border border-slate-800">
            {currentTrip.stores.length} Stops Total
          </span>
        </h3>
        ...
        <div className="space-y-3">
          {currentTrip.stores.map((store, index) => {
            ...
            return (
              <div 
                key={store.id}
                className="p-4 sm:p-5 rounded-2xl bg-[#07090e] border border-slate-800/90 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
              >
                <div className="flex items-start gap-3.5">
                  <div className="w-8 h-8 rounded-xl bg-blue-500/20 border border-blue-500/40 text-blue-300 font-extrabold text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-mono">
                    {index + 1}
                  </div>
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-bold text-sm text-white font-display">{store.name}</span>
                ...
                {storeDeal && (
                  <div className="sm:text-right flex-shrink-0 p-3 rounded-xl bg-[#0b0f19] border border-slate-800">
                    <span className="text-[10px] text-slate-400 block font-semibold uppercase tracking-wider">Target Item:</span>
                    <strong className="text-xs text-amber-300 block truncate max-w-[220px]">
                      {storeDeal.title}
                    </strong>
                    <div className="text-xs font-mono mt-1 flex items-center sm:justify-end gap-2">
                      <span className="text-white">${storeDeal.inStorePrice.toFixed(2)} Buy</span>
                      <span className="text-emerald-400 font-bold">+{storeDeal.profitMarginPercent.toFixed(0)}% ROI</span>
                    </div>
                  </div>
                )}
              </div>
